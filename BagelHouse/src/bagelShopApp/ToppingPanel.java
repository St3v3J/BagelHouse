...
private final String[] TOPPING_TYPE={"Cream Cheese","Butter","PeachJelly","Blueberry jam"};
...
public ToppingPanel()
{
// Create a GridLayout manager with four rows and one column. setLayout(new
//GridLayout(4, 1));
// Create the check boxes and add them into the array toppingOptions
// Add the check boxes to panel
for (int i = 0; i < toppingOptions.length; i++)
{
toppingOptions[i] = new JCheckBox(TOPPING_TYPE[i]); add(toppingOptions[i]);
}
...
for (int i = 0; i < toppingOptions.length; i++)
if (toppingOptions[i].isSelected()) toppingCost += TOPPING_COSTS[i];
return toppingCost;
}
} 